...
class Calibration:
    ...
    def xplot(self):
        ...
        vdat = self.ts['x']
        xdat = np.arange(len(vdat)) / self.rate
        xplt.plot(xdat, vdat)
    ...
    def yplot(self):
        ...
        vdat = self.ts['y']
        ydat = np.arange(len(vdat)) / self.rate
        yplt.plot(ydat, vdat)
    ...
    def plot_band_stop(self, low_f, high_f, axis='x', order=6):
        fig = plt.figure()
        xplt = fig.add_subplot(111)
        vdat = self.ts[axis]
        tdat = np.arange(len(vdat)) / self.rate
        xplt.plot(tdat, vdat)
        filtered = self.band_stop(low_f, high_f, axis, order)
        xplt.plot(tdat, filtered)
        xplt.set_xlabel("Time (s)")
        xplt.set_ylabel("Voltage (V)")
        xplt.set_title(
